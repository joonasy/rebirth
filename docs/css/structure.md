...
Here is listed default partial structure used across projects:
    ...
    |   …
    | 
    |── vendors/               # CSS or Sass from other projects [6]
    |   |── _datepicker.scss
    |   |── _slick.scss
    |   …
    |
...
* **6.** 3rd party vendors are located here only if they aren't available in [Bower](http://bower.io). Place modified bower css components here.
